...
def subtask2(check_with_external_library=False):
    ...
    Execute subtask 2
    """
    swiss_roll = swissroll_dataset(n=5000)
    swiss_roll_small = swissroll_dataset(n=1000)
    color = swissroll_color(swiss_roll)
    color_small = swissroll_color(swiss_roll_small)
    plot_swissroll(swiss_roll, color, "swiss_roll")
    plot_swissroll(swiss_roll_small, color_small, "swiss_roll_small")
    ...
    if check_with_external_library:
        ...
        plot_subtask2(phi_l, color, "eigenfunctions", 1)

        lambda_l, phi_l = diffusion_map(swiss_roll_small, l=10)
        # plot_dmaps(phi_l, "task2_subtask2_eigenfunctions")
        plot_subtask2(phi_l, color_small, "eigenfunctions_n1000", 1)

    pc_coordinates, u, s, vh = pca(swiss_roll)
    plot_subtask2(pc_coordinates, color, "pca_n5000")
    pc_coordinates, u, s, vh = pca(swiss_roll_small)
    plot_subtask2(pc_coordinates,color_small, "pca_n1000")


def swissroll_dataset(n=1000) -> np.ndarray:
    """
...
def plot_swissroll(swiss_roll, color, name):
    """
...
def plot_subtask2(dmap, color, name, offset=0):
    fig, ax = plt.subplots(dmap.shape[1]-1-offset)
    for i in range(dmap.shape[1]-offset):
        if i == 0:
            continue
        ax[i-1].scatter(dmap[:, offset], dmap[:, i+offset],
                        s=(72./fig.dpi)**2, c=color)
        ax[i-1].set_xlabel("$\phi_{}$".format(offset+1))
        ax[i-1].set_ylabel("$\phi_{}$".format("{"+str(i+offset+1)+"}"))
    plt.show()
    plt.pause(0.5)
    fig.set_size_inches(8, 8)
    fig.savefig('task2_subtask2_{}.png'.format(name), dpi=100)
